classdef LinearElasticTL_planeStress < LinearElasticTL
% Version for 
    
    methods
        
        function obj = LinearElasticTL_planeStress(ID,rho,E,nu)
            % Inputs: ID - INT
            %         rho - FLOAT
            %         E - FLOAT
            %         nu - FLOAT
            ...
            lambdabar = (2*obj.lambda*obj.mu)/(obj.lambda+2*obj.mu); % lambda for plane stress case (FLOAT)
            obj.c = sqrt(E/((1-nu^2)*rho)); % sound speed (FLOAT)
            obj.C = [(lambdabar+2*obj.mu)     lambdabar          0;...
                           lambdabar    (lambdabar+2*obj.mu)     0;...
